...
Reserved Notation "'⊨' t '⟼' t1" (at level 57, t custom rsf, 
                                                   t1 custom rsf, no associativity).
Reserved Notation "'⊨' t '⟼⋆' t1" (at level 57, t custom rsf, 
                                                    t1 custom rsf, no associativity).
Reserved Notation "'⊨' t '⊢' st '⟶' t1" (at level 57, t custom rsf, 
                                                         t1 custom rsf, no associativity).
...
(** *** Proof of preservation of typing through an evaluation transition *)
Theorem evaluate_preserves_typing : forall Re t t' τ,
  ∅ᵥᵪ ⋅ Re ⊫ t ∈ τ -> ⊨ t ⟼ t' -> 

  ∅ᵥᵪ ⋅ Re ⊫ t' ∈ τ.
Proof. 
  intros Re t t' τ Hwt; generalize dependent t'; dependent induction Hwt; intros t' HeTt;
  inversion HeTt; subst; try (econstructor; now eauto); eauto.
  - inversion Hwt1; subst. eapply subst_preserves_typing; eauto. apply RContext.Submap_refl.
...
Theorem multi_preserves_typing : forall Re t t' τ,
  ...
  ∅ᵥᵪ ⋅ Re ⊫ t' ∈ τ.
Proof.
  intros Re t t' τ Hwt HmeT; dependent induction HmeT; subst; try assumption.
  apply IHHmeT; auto. eapply (evaluate_preserves_typing Re x y τ Hwt H).
Qed.
...
Proof.
  ...
  - destruct H3',H5'; right. 
    -- inversion H; subst; inversion H3; subst. exists <[[x:= t2] t]>.
      now constructor.
  ...
  - apply IHt in H4 as H4'; destruct H4' as [Hvt | [t' HeT']]; 
